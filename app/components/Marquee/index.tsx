    ...
    <div className="relative col-span-full w-full overflow-hidden py-6">
      ...
      >
        {[...ICONS, ...ICONS].map((Icon: React.ReactNode, index) => (
          <div
            key={`icon-${index}`}
            className="border-greyscale-200/50 dark:border-greyscale-700 transiton-[rotate,scale] ease-in-out-circ dark:bg-greyscale-950/50 bg-greyscale-50/50 text-greyscale-600 dark:text-greyscale-300 mr-3 aspect-square h-fit w-fit rounded-2xl border p-3 shadow-[var(--nav-bar-light)] backdrop-blur-sm duration-300 hover:scale-105 hover:odd:-rotate-4 hover:even:rotate-4 md:mr-4 md:p-4 dark:shadow-[var(--nav-bar-dark)]"
          >
            {Icon}
          </div>
        ))}
